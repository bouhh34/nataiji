...
const UI_KEY='nataiji-ui-state-v1',DATA_KEY='nataiji-data';
...
try{if(typeof normalizeState==='function'&&!window.__nataijiNormalizePatched){window.__nataijiNormalizePatched=true;const original=normalizeState;normalizeState=function(serverState){let local=null;try{local=JSON.parse(localStorage.getItem(DATA_KEY)||'null')}catch{}return original(local&&typeof local==='object'?local:serverState)}}}catch{}
...
function saveUi(){writeUi({view:currentView(),report:currentReport(),student:q('#student')?.value??'',subject:q('#subjectPicker')?.value??'',term:q('#term')?.value??'',classTop:q('#classTop')?.value??'',yearTop:q('#yearTop')?.value??'',scrollY:Math.max(0,window.scrollY||0)});persistData()}
...
document.addEventListener('input',e=>{if(e.target.matches?.('.mark,.mobile-mark,input,select,textarea')){persistData();writeUi({student:q('#student')?.value??'',subject:q('#subjectPicker')?.value??'',term:q('#term')?.value??'',classTop:q('#classTop')?.value??'',yearTop:q('#yearTop')?.value??''})}},true);
...
window.addEventListener('DOMContentLoaded',()=>{settle(0);settle(100);settle(350);setTimeout(()=>document.documentElement.classList.add('nataiji-restored'),450)});setTimeout(()=>{persistData();settle(0)},0);
})();