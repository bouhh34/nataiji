...
function saveUi(){writeUi({view:currentView(),report:currentReport(),student:q('#student')?.value??'',subject:q('#subjectPicker')?.value??'',scrollY:Math.max(0,window.scrollY||0)})}
...
function restoreUi(){const ui=readUi();try{if(ui.view&&typeof setView==='function')setView(ui.view)}catch{}try{if(ui.report&&typeof showReport==='function')showReport(ui.report)}catch{}restoreSelect('#student',ui.student,()=>{try{renderReports()}catch{}});restoreSelect('#subjectPicker',ui.subject,()=>{try{renderMobileScores()}catch{}});if(Number.isFinite(Number(ui.scrollY)))requestAnimationFrame(()=>window.scrollTo(0,Number(ui.scrollY)));document.documentElement.classList.add('nataiji-restored');document.documentElement.removeAttribute('data-nataiji-restore-view')}
...
document.addEventListener('input',e=>{if(e.target.matches?.('.mark,.mobile-mark,input,select,textarea'))writeUi({student:q('#student')?.value??'',subject:q('#subjectPicker')?.value??''})},true);
document.addEventListener('change',e=>{saveUi();if(e.target.matches?.('#student,#subjectPicker'))settle(40)},true);
