 ...
 const draw=async()=>{
  ...
  box.innerHTML=grants.map((g,gi)=>{
   ...
   const classHtml=(details.length?details:fallbackIds.map(id=>({id,name:id,editSubjects:[],viewSubjects:[],hiddenSubjects:[],fullClass:!!g.classAccess?.[id]?.fullClass}))).map(d=>{
    ...
    return `<div class="auth2-share-class-detail"><div class="auth2-share-class-head"><b>${esc3(d.name||d.code||tr('قسم','Classe'))}</b>${d.nameFr?`<small dir="ltr">${esc3(d.nameFr)}</small>`:''}</div><div class="auth2-share-class-badges">${permissionBadge(grades,esc3(gradeText),esc3(gradeText))}${permissionBadge(perms.has('pupils'),tr('التلاميذ: مسموح','Élèves : autorisés'),tr('التلاميذ: غير مسموح','Élèves : non autorisés'))}${permissionBadge(perms.has('reports'),tr('التقارير: مسموح','Rapports : autorisés'),tr('التقارير: غير مسموح','Rapports : non autorisés'))}</div>${edit.length?`<p><strong>${tr('مواد قابلة للتعديل:','Matières modifiables :')}</strong> ${esc3(edit.join('، '))}</p>`:''}${hidden.length?`<p><strong>${tr('مواد مخفية:','Matières masquées :')}</strong> ${esc3(hidden.join('، '))}</p>`:''}${view.length&&grades?`<p><strong>${tr('عرض فقط:','Lecture seule :')}</strong> ${esc3(view.join('، '))}</p>`:''}</div>`
   }).join('');
...
.auth2-share-audit-card>header{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:10px}
.auth2-share-audit-card>header>div{display:grid;gap:3px;min-width:0}.auth2-share-audit-card>header b{font-size:15px;color:#223b4d}
.auth2-share-audit-card>header small{font-size:11.5px;color:#718392}.auth2-share-audit-card .auth2-revoke{flex:0 0 auto;padding:8px 10px;border-radius:9px}
