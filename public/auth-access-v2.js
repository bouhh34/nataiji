...
function accountActivate(user){const prev=localStorage.getItem('nataiji-active-user');if(prev&&prev!==user.id)localStorage.removeItem('nataiji-data');localStorage.setItem('nataiji-active-user',user.id);currentUser=user;const g=q('.auth-gate');if(g)g.remove();document.documentElement.classList.add('nataiji-auth-ready');const routed=window.NataijiProfessor?.activate?.(user);if(routed)return routed;return startApp()}
...
function registerForm(){return `<form class="auth-form auth2-form" id="auth2Form"><label>${tr('الاسم','Nom')}<input name="name" autocomplete="name" required></label><label>${tr('البريد الإلكتروني','Adresse e-mail')}<input name="email" type="email" autocomplete="email" required></label><label>${tr('كلمة المرور','Mot de passe')}<input name="password" type="password" minlength="8" autocomplete="new-password" required></label><button class="auth-submit">${tr('إنشاء الحساب','Créer le compte')}</button><p class="auth2-note">${tr('بعد إنشاء الحساب ستختار نوعه مرة واحدة ثم تكمل الإعداد.','Après la création, vous choisirez une seule fois le type de compte puis terminerez la configuration.')}</p><p class="auth-error"></p></form>`}
...
async function schoolsModal(){
 const p=modal(tr('مدارسي','Mes écoles'),`<p>${tr('كل مدرسة مساحة مستقلة تمامًا بأقسامها وتلاميذها ونتائجها.','Chaque école est un espace totalement indépendant avec ses classes, élèves et résultats.')}</p><div id="auth2Schools"><p class="message">${tr('جارٍ التحميل…','Chargement…')}</p></div><hr><h3>${tr('إضافة مدرسة','Ajouter une école')}</h3><label>${tr('اسم المدرسة بالعربية','Nom de l’école en arabe')}<input id="auth2SchoolAr" dir="rtl"></label><label>Nom de l’école en français<input id="auth2SchoolFr" dir="ltr"></label><button class="primary auth2-school-create" id="auth2AddSchool">${tr('إضافة وفتح المدرسة','Ajouter et ouvrir l’école')}</button><p class="message" id="auth2SchoolMsg"></p>`);
 const schoolAr=q('#auth2SchoolAr',p),schoolFr=q('#auth2SchoolFr',p);
 const toFr=v=>window.nataijiArToFr?window.nataijiArToFr(v):String(v||'').trim();
 const toAr=v=>window.nataijiFrToAr?window.nataijiFrToAr(v):String(v||'').trim();
 schoolAr.addEventListener('input',()=>{schoolFr.value=toFr(schoolAr.value)});
 schoolFr.addEventListener('input',()=>{schoolAr.value=toAr(schoolFr.value)});
...
function bindShares(){
 ...
 stableMarkup(b,`<b><i data-feather="share-2" aria-hidden="true"></i>${tr('المشاركات','Partages')}</b><span>${tr('عرض من لديه وصول وإلغاء الصلاحية عند الحاجة','Voir les accès accordés et les retirer si nécessaire')}</span>`);b.onclick=sharesModal
}
...
function bindJoinCard(){
 ...
 stableMarkup(btn,`<b><i data-feather="link" aria-hidden="true"></i>${tr('إضافة قسم مشترك برمز','Ajouter une classe partagée par code')}</b><span>${tr('أضف قسمًا أو مواد جديدة إلى حسابك الحالي دون تسجيل الخروج','Ajoutez une classe ou de nouvelles matières à votre compte sans vous déconnecter')}</span>`);
