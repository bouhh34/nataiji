...
function ownerOverviewModal(){
 ...
 api('/api/owner/overview').then(r=>{const s=r.stats||{},labels=[['users','الحسابات','Comptes'],['schools','المدارس','Écoles'],['schoolAdmins','مديرو المدارس','Directeurs'],['teachers','المعلمون','Enseignants']];x.querySelector('.owner-loading').outerHTML=`<div class="owner-stats">${labels.map(([k,ar,ff])=>`<article><strong>${Number(s[k]||0)}</strong><span>${tr(ar,ff)}</span></article>`).join('')}</div><div class="owner-accounts"><h3>${tr('حسابات المنصة','Comptes de la plateforme')}</h3>${(r.accounts||[]).map(a=>`<div><span><b>${esc3(a.name||tr('دون اسم','Sans nom'))}</b><small dir="ltr">${esc3(a.email)}</small></span><em class="owner-role ${a.role}">${a.role==='owner'?tr('المدير العام','Super administrateur'):a.role==='teacher'?tr('معلم','Enseignant'):tr('مدير مدرسة','Directeur')}</em></div>`).join('')||`<p>${tr('لا توجد حسابات بعد.','Aucun compte pour le moment.')}</p>`}</div>`}).catch(()=>{const el=x.querySelector('.owner-loading');if(el)el.textContent=tr('تعذر تحميل الملخص الآن.','Impossible de charger la vue d’ensemble pour le moment.')});
}
