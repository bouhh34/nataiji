...
function subjectOptions(catalog,level,branch='',selected=''){
 const l=(catalog?.levels||[]).find(x=>x.code===level),b=(l?.branches||[]).find(x=>x.code===String(branch||'').toUpperCase()),subjects=b?.subjects?.length?b.subjects:(l?.subjects||[]);return subjects.map(s=>`<option value="${esc(s.key)}" ${s.key===selected?'selected':''}>${esc(fr()?s.fr:s.ar)} · ×${s.coefficient}</option>`).join('')
}
...
function addClassModal(catalog,onSave){
 const x=modal(t('إضافة قسم للأستاذ','Ajouter une classe au professeur'),`<label>${t('اسم القسم','Nom de la classe')}<input id="staffClassName" maxlength="120" placeholder="2AS-A"></label><label>${t('المستوى','Niveau')}<select id="staffClassLevel">${curriculumOptions(catalog,'1AS')}</select></label><label id="staffClassBranchLabel" style="display:none">${t('الشعبة الرسمية','Section officielle')}<select id="staffClassBranch"></select><small>${t('للثانوية اختر A أو C أو D أو O حسب القسم.','Au lycée, choisissez A, C, D ou O selon la classe.')}</small></label><button class="primary" id="staffClassSave">${t('إضافة القسم','Ajouter la classe')}</button><p class="message"></p>`);
 const levelSel=q('#staffClassLevel',x),branchLabel=q('#staffClassBranchLabel',x),branchSel=q('#staffClassBranch',x);
 const syncBranch=()=>{const l=(catalog?.levels||[]).find(v=>v.code===levelSel.value),branches=Array.isArray(l?.branches)?l.branches:[];branchLabel.style.display=branches.length?'grid':'none';branchSel.innerHTML=branches.map(b=>`<option value="${esc(b.code)}">${esc(b.code)} — ${esc(fr()?b.fr:b.ar)} · Σ${b.expectedCoefficientTotal}</option>`).join('')};
 levelSel.onchange=syncBranch;syncBranch();
 q('#staffClassSave',x).onclick=async()=>{const name=q('#staffClassName',x).value.trim(),levelCode=levelSel.value,l=(catalog?.levels||[]).find(v=>v.code===levelCode),branches=Array.isArray(l?.branches)?l.branches:[],branchCode=branches.length?branchSel.value:'',msg=q('.message',x);if(!name){msg.textContent=t('اكتب اسم القسم.','Saisissez le nom de la classe.');return}if(branches.length&&!branchCode){msg.textContent=t('اختر الشعبة الرسمية.','Choisissez la section officielle.');return}try{await onSave({name,levelCode,branchCode});x.remove()}catch(e){msg.textContent=e.code==='professor_class_exists'?t('هذا القسم موجود بالفعل.','Cette classe existe déjà.'):e.code==='invalid_branch'?t('الشعبة غير مطابقة للمستوى.','Section invalide pour ce niveau.'):t('تعذر إضافة القسم.','Impossible d’ajouter la classe.')}}
}
function addAssignmentModal(catalog,classes,onSave){
 ...
 const sync=()=>{const opt=clsSel.selectedOptions[0],level=opt?.dataset.level||'',branch=opt?.dataset.branch||'';subSel.innerHTML=subjectOptions(catalog,level,branch)+`<option value="__other__">${t('مادة أخرى…','Autre matière…')}</option>`;syncSub()};
 const syncSub=()=>{const other=subSel.value==='__other__';customLabel.style.display=other?'grid':'none';coefLabel.style.display=other?'grid':'none';hint.textContent=other?t('المادة الإضافية لا تدخل ضمن المجموع الرسمي لمعاملات القسم.','Une matière ajoutée manuellement ne compte pas dans le total officiel des coefficients.'):t('المعامل الرسمي سيطبق تلقائيًا.','Le coefficient officiel sera appliqué automatiquement.')};
