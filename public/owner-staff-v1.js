...
function renderStaff(root,overview,staff,tab){
 const host=q('.staff-loading,.staff-content',root);if(!host)return;const s=overview.stats||{},items=tab==='professor'?staff.professors:staff.teachers;
 host.outerHTML=`<div class="staff-content">
  ...
  <div class="staff-toolbar"><div><h3>${tab==='professor'?t('الأساتذة','Professeurs'):t('المعلمون','Enseignants')}</h3><p>${tab==='professor'?t('المواد والأقسام والمعاملات وحالة الربط، دون عرض الدرجات الخام.','Matières, classes, coefficients et liaisons, sans afficher les notes brutes.'):t('حالة حسابات المعلمين وصلاحيات الوصول العامة.','État des comptes enseignants et accès généraux.')}</p></div><input id="staffSearch" type="search" placeholder="${t('بحث بالاسم أو البريد…','Rechercher par nom ou e-mail…')}"></div>
  <div class="staff-list">${items.map(a=>card(a,tab)).join('')||`<div class="staff-empty">${t('لا توجد حسابات في هذا القسم.','Aucun compte dans cette catégorie.')}</div>`}</div>
 </div>`;
 qa('[data-staff-tab]',root).forEach(b=>b.onclick=()=>renderStaff(root,overview,staff,b.dataset.staffTab));
 const search=q('#staffSearch',root);search.oninput=()=>{const v=search.value.trim().toLowerCase();qa('.staff-card',root).forEach(row=>row.style.display=row.textContent.toLowerCase().includes(v)?'':'none')};
