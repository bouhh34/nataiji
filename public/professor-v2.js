...
function professorStudentFullyAbsent(row){
 const results=Array.isArray(row?.subjectResults)?row.subjectResults:[];
 return results.length>0&&results.every(r=>professorIsAbsent(r?.tests?.[0])&&professorIsAbsent(r?.exam))
}
...
function openGrades(id,term=1){
 ...
 const el=root();el.innerHTML=`<div class="professor-shell prof-grade-reference">${topbar(tr('إدخال الدرجات','Saisie des notes'),l?tr('قسم مشترك','Classe partagée'):tr('موادي','Mes matières'))}
  ...
  <section class="professor-content prof-grade-teacher-style">
   ...
   <div class="prof-grade-absence-tip"><span>i</span><p>${tr('غائب = 0 في الحساب، وإذا غاب عن الاختبار والامتحان يظهر المعدل «غائب».','Absent = 0 dans le calcul ; absent aux deux épreuves = moyenne « Absent ».')}</p></div>
...
function compactResultText(v){
 if(v==null||v==='')return'—';const n=Number(v);if(!Number.isFinite(n))return'—';return Number.isInteger(n)?String(n):n.toFixed(2).replace(/0+$/,'').replace(/\.$/,'')
}
function professorCollectiveSubjectCell(result,term){
 if(Number(term)!==3&&professorIsAbsent(result?.tests?.[0])&&professorIsAbsent(result?.exam))return '<span class="collective-absent"><b>غائب</b><small>Abs.</small></span>';
 return compactResultText(Number(term)===3?result?.annualAverage:result?.average)
}
...
function printProfessorDocument(title,body,opts={}){
 ...
 @page{size:${pageSize};margin:${pageMargin}}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;color:#111;font-family:Tahoma,Arial,sans-serif}html,body{width:${landscape?'297mm':'210mm'};min-height:${landscape?'210mm':'297mm'}}body{direction:ltr}.paper{width:100%;max-width:${paperWidth};margin:0 auto;padding:0 3mm;direction:ltr}.official-head{display:grid;grid-template-columns:minmax(0,1fr) 34mm minmax(0,1fr);grid-template-areas:"fr center ar";gap:5mm;align-items:start;width:100%;font-size:8.7pt;line-height:1.34;margin-bottom:3mm}.official-side{min-width:0;display:flex;flex-direction:column;gap:.35mm}.official-fr{grid-area:fr;text-align:left;direction:ltr}.official-ar{grid-area:ar;text-align:right;direction:rtl}.official-strong{font-weight:800}.official-center{grid-area:center;display:flex;flex-direction:column;align-items:center;text-align:center}.basmala{width:100%;font-size:8.7pt;font-weight:800;line-height:1.3;margin:0 0 .8mm;white-space:nowrap;text-align:center;direction:rtl}.official-center img{width:23mm;height:23mm;object-fit:contain}.report-title{display:flex;justify-content:center;align-items:center;gap:4mm;text-align:center;font-size:15.5pt;margin:2mm 0 1.2mm;font-weight:800}.report-title .dual-fr{font-size:.82em}.report-subtitle{text-align:center;font-size:8.5pt;margin:0 0 2mm;color:#333}.report-subtitle .dual-line{display:flex;justify-content:center;gap:3mm}.student-name{display:grid;grid-template-columns:1fr 1fr;gap:0;border:1.2px solid #111;padding:0;margin:.8mm 0 0;font-size:9.6pt}.student-name>span{padding:1.15mm 1.7mm}.student-name>span+span{border-left:1px solid #222}.student-name [dir=rtl]{text-align:right}.student-name [dir=ltr]{text-align:left}.student-info{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0;font-size:7.5pt;margin:0 0 1.2mm;border-left:1.2px solid #111;border-top:0}.student-info .dual-info{display:flex;justify-content:space-between;gap:2mm;border-right:1px solid #222;border-bottom:1px solid #222;padding:.75mm 1mm;min-height:5.2mm}.student-info .dual-info span:last-child{text-align:right}.result-table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:7.4pt;direction:ltr;border:1.25px solid #111}.result-table th,.result-table td{border:1px solid #222;padding:.8mm .55mm;text-align:center;vertical-align:middle}.result-table th{font-weight:800;background:#fff}.result-table thead tr:first-child th{border-top:1.25px solid #111}.result-table tr>*:first-child{border-left:1.25px solid #111}.result-table tr>*:last-child{border-right:1.25px solid #111}.result-table tbody tr:last-child td{border-bottom:1.25px solid #111}.result-table .name{text-align:center;font-weight:700}.result-table .name .dual-ar,.result-table .name .dual-fr,.result-table th .dual-ar,.result-table th .dual-fr{display:block;line-height:1.15}.result-table .name .dual-fr,.result-table th .dual-fr{font-size:.84em;font-weight:600;margin-top:.35mm}.result-table small{font-size:6.4pt}.secondary-bulletin-table{font-size:6.7pt}.secondary-bulletin-table thead th{padding:.7mm .35mm;line-height:1.05}.secondary-bulletin-table .discipline-head{width:20%}.secondary-bulletin-table .assessment-col{width:5.5%}.secondary-bulletin-table .mean-head,.secondary-bulletin-table .coef-head,.secondary-bulletin-table .weighted-head{width:7%}.secondary-bulletin-table .obs-head{width:14%}.secondary-bulletin-table .group-head{font-size:7pt}.secondary-bulletin-table td{padding:.8mm .35mm;height:6.1mm}.secondary-bulletin-table .observation-cell .dual-fr,.secondary-bulletin-table .observation-cell .dual-ar{font-size:.78em}.secondary-summary{display:grid;grid-template-columns:1fr 1.35fr .8fr;gap:0;margin-top:0;border:1.2px solid #111;border-top:0}.secondary-summary>div{display:grid;grid-template-columns:1fr auto;align-items:center;gap:1.5mm;padding:.9mm 1.3mm;border-inline-end:1px solid #222;font-size:7.4pt}.secondary-summary>div:last-child{border-inline-end:0}.secondary-summary b .dual-ar,.secondary-summary b .dual-fr{display:block}.secondary-summary strong{font-size:8.3pt}.secondary-observation{margin-top:0;border:1.2px solid #111;border-top:0;padding:1mm 1.3mm;display:grid;grid-template-columns:auto 1fr 1fr;align-items:center;gap:2mm;min-height:7mm;font-size:7.4pt}.secondary-observation b .dual-ar,.secondary-observation b .dual-fr{display:block}.dual-ar{direction:rtl}.dual-fr{direction:ltr}.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:2mm;margin:2.5mm 0 0;font-size:8.8pt;font-weight:800;text-align:center}.summary span{display:block}.summary .dual-summary{border:1px solid #ddd;padding:1.2mm;border-radius:2mm}.summary .dual-summary .dual-fr{font-size:.82em;margin-top:.4mm}.incomplete{font-size:7.1pt;text-align:center;color:#111;margin:1mm 0;display:grid;gap:.35mm}.official-signatures{display:grid;grid-template-columns:1fr .72fr 1fr;gap:9mm;margin:3.5mm 8mm 0;text-align:center;break-inside:avoid;direction:rtl;align-items:end}.official-signatures>div{display:flex;flex-direction:column;align-items:center;gap:.35mm}.official-signatures b{font-size:8.2pt}.official-signatures span{font-size:6.9pt}.official-signatures small{font-size:6.1pt;font-weight:600}.official-signatures i{display:block;width:30mm;height:4.5mm;margin:auto;border-bottom:1px dotted #444}.official-date-line{align-self:start;padding-top:1.2mm}.official-date-line span{font-weight:700;letter-spacing:.25mm}.stamp-zone{width:22mm;height:9mm;border:1px dashed #777;display:grid!important;place-items:center;font-size:5.8pt;margin:.4mm 0;color:#333}.director-zone i{height:2.5mm}.compact .official-signatures{margin-top:5mm}.compact .report-title{font-size:14pt;margin-top:1.5mm}.few .result-table{font-size:8.8pt}.few .result-table th,.few .result-table td{padding:1.35mm .9mm;height:8mm}.few .official-signatures{margin-top:9mm}.medium .result-table{font-size:7.5pt}.medium .result-table th,.medium .result-table td{padding:.9mm .55mm;height:6.8mm}.dense .result-table{font-size:6.2pt}.dense .result-table th,.dense .result-table td{padding:.45mm .28mm;height:5.5mm}.dense .official-signatures{margin-top:3.5mm}.class-list-doc .result-table{font-size:9pt}.class-list-doc .result-table th,.class-list-doc .result-table td{padding:1.35mm .85mm}.class-list-doc .report-subtitle{font-size:9pt}.professor-own-list-doc .result-table{font-size:8.4pt}.professor-own-list-doc .result-table th,.professor-own-list-doc .result-table td{padding:1.15mm .65mm}.professor-own-list-doc .report-subtitle{font-size:8.6pt;margin-bottom:2mm}.professor-own-list-doc .own-list-annual-note{font-size:7.2pt;text-align:center;margin:2mm 0 0}.professor-own-list-portrait{padding:0 1.5mm}.professor-own-list-portrait .official-head{grid-template-columns:minmax(0,1fr) 31mm minmax(0,1fr);gap:4mm;font-size:9pt;line-height:1.3;margin-bottom:1.5mm}.professor-own-list-portrait .official-center img{width:24mm;height:24mm}.professor-own-list-portrait .basmala{font-size:9pt;margin-bottom:.45mm}.professor-own-list-portrait .report-title{font-size:16pt;margin:.8mm 0 .8mm}.professor-own-list-portrait .report-subtitle{font-size:9pt;margin-bottom:1.4mm}.professor-own-list-portrait .result-table{font-size:8.8pt}.professor-own-list-portrait .result-table th,.professor-own-list-portrait .result-table td{padding:1.35mm .7mm;height:7.6mm}.professor-own-list-portrait .result-table tr>*:first-child{width:7%}.professor-own-list-portrait .result-table tr>*:nth-child(2){width:19%}.professor-own-list-portrait.professor-own-list-1-subject .result-table{font-size:10pt}.professor-own-list-portrait.professor-own-list-1-subject .result-table th,.professor-own-list-portrait.professor-own-list-1-subject .result-table td{padding:1.7mm 1mm;height:8.8mm}.professor-own-list-portrait.professor-own-list-2-subject .result-table{font-size:8.7pt}.professor-own-list-portrait.professor-own-list-2-subject .result-table th,.professor-own-list-portrait.professor-own-list-2-subject .result-table td{padding:1.35mm .6mm}.professor-own-list-portrait .official-signatures{margin-top:7mm}.professor-own-list-portrait .official-signatures b{font-size:8.8pt}.professor-own-list-portrait .official-signatures span{font-size:7.5pt}.professor-own-list-portrait .official-signatures small{font-size:6.8pt}.professor-own-list-portrait .stamp-zone{width:25mm;height:10mm}.professor-own-list-portrait .own-list-annual-note{font-size:8pt;margin-top:2.5mm}.class-list-doc{padding:0 .5mm}.class-list-doc .official-head{grid-template-columns:minmax(0,1fr) 40mm minmax(0,1fr);gap:5.5mm;font-size:10.6pt;line-height:1.32;margin-bottom:2mm}.class-list-doc .official-side{gap:.5mm}.class-list-doc .official-fr{font-size:10.6pt}.class-list-doc .official-ar{font-size:11.5pt;line-height:1.34}.class-list-doc .official-fr .official-strong{font-size:11pt}.class-list-doc .official-ar .official-strong{font-size:11.8pt}.class-list-doc .official-strong{font-weight:800}.class-list-doc .official-center img{width:29mm;height:29mm}.class-list-doc .basmala{font-size:10.8pt}.class-list-doc .report-title{font-size:18.5pt;margin:1mm 0}.class-list-doc .report-subtitle{font-size:10.5pt;margin-bottom:2.2mm}.class-list-doc .collective-meta{display:grid;gap:.7mm}.class-list-doc .collective-meta-row{display:flex;justify-content:center;align-items:center;gap:5mm;flex-wrap:wrap}.class-list-doc .collective-meta-row.ar{font-size:10.8pt}.class-list-doc .collective-meta-row.fr{font-size:9.2pt}.class-list-doc .collective-meta-row span{white-space:nowrap}.class-list-doc .collective-meta-row b{font-weight:800}.class-list-doc .incomplete{font-size:8.4pt;margin:.8mm 0 1.2mm}.class-list-doc .result-table{font-size:10pt}.class-list-doc .result-table th,.class-list-doc .result-table td{padding:1.8mm .55mm;height:9mm}.class-list-doc .collective-class-table .rank-head,.class-list-doc .collective-class-table .rank-cell{width:5.5%;font-weight:800}.class-list-doc .collective-class-table .student-head,.class-list-doc .collective-class-table td.name:nth-child(2){width:21%}.class-list-doc .collective-class-table .average-head{width:8%}.class-list-doc .collective-class-table .appreciation-head,.class-list-doc .collective-class-table .appreciation-cell{width:11.5%}.class-list-doc .collective-class-table th.subject-head{height:39mm;padding:.7mm .15mm;vertical-align:middle}.class-list-doc .collective-class-table .subject-vertical{height:30mm;display:flex;align-items:center;justify-content:center;writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);font-size:9.2pt;font-weight:800;line-height:1.05;white-space:nowrap;margin:auto}.class-list-doc .collective-class-table th.subject-head b{display:block;font-size:8pt;margin-top:.45mm}.class-list-doc .collective-class-table th.subject-head small{display:block;font-size:6.8pt;margin-top:.25mm}.class-list-doc .collective-class-table{table-layout:fixed;width:100%}.class-list-doc .collective-class-table .student-head,.class-list-doc .collective-class-table td.name:nth-child(2){width:19.5%}.class-list-doc .collective-class-table .average-head{width:7.5%}.class-list-doc .collective-class-table .appreciation-head,.class-list-doc .collective-class-table .appreciation-cell{width:11.5%}.class-list-doc .collective-class-table .edge-vertical-head{height:39mm;padding:.7mm .15mm;vertical-align:middle;overflow:hidden}.class-list-doc .collective-class-table .edge-vertical-label{height:31mm;display:flex;align-items:center;justify-content:center;gap:1.2mm;writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);margin:auto;white-space:nowrap;overflow:hidden}.class-list-doc .collective-class-table .edge-vertical-label b{font-size:9.3pt;line-height:1;font-weight:800}.class-list-doc .collective-class-table .edge-vertical-label small{font-size:6.7pt;line-height:1;font-weight:650}.class-list-doc .collective-class-table .appreciation-cell{padding-inline:.7mm;white-space:normal;overflow-wrap:anywhere;word-break:normal;line-height:1.12}.class-list-doc .collective-class-table .appreciation-cell .dual-ar,.class-list-doc .collective-class-table .appreciation-cell .dual-fr{white-space:normal;overflow-wrap:anywhere}.class-list-doc .collective-class-table td.subject-value{padding-inline:.22mm;font-size:9pt;font-weight:700;white-space:nowrap;overflow:hidden}.class-list-doc .collective-class-table td.general-value{font-weight:800;white-space:nowrap}.class-list-doc .collective-class-table .collective-absent{display:grid;place-items:center;gap:.15mm;line-height:1}.class-list-doc .collective-class-table .collective-absent b{font-size:7.8pt;line-height:1;font-weight:800}.class-list-doc .collective-class-table .collective-absent small{font-size:5.7pt;line-height:1;color:#444}.class-list-doc.class-list-students-few .collective-class-table td.subject-value{font-size:9.8pt}.class-list-doc.class-list-students-medium .collective-class-table td.subject-value{font-size:8.8pt}.class-list-doc.class-list-students-many .collective-class-table td.subject-value{font-size:7.7pt}.class-list-doc .official-signatures{margin-top:6mm}.class-list-doc .official-signatures b{font-size:11pt}.class-list-doc .official-signatures span{font-size:9.1pt}.class-list-doc .official-signatures small{font-size:8.2pt}.class-list-doc .official-signatures i{width:34mm;height:5.5mm}.class-list-doc .official-date-line{padding-top:.6mm}.class-list-doc .official-date-line span{font-size:9.3pt;letter-spacing:.15mm}.class-list-doc.class-list-students-few .result-table{font-size:12.5pt}.class-list-doc.class-list-students-few .result-table th,.class-list-doc.class-list-students-few .result-table td{padding:2.8mm .5mm;height:14mm}.class-list-doc.class-list-students-few .collective-class-table th:not(.subject-head) b{font-size:11.8pt}.class-list-doc.class-list-students-few .collective-class-table .subject-vertical{font-size:9.6pt}.class-list-doc.class-list-students-few .collective-class-table th.subject-head b{font-size:8.1pt}.class-list-doc.class-list-students-few .collective-class-table th.subject-head small{font-size:6.9pt}.class-list-doc.class-list-students-few .collective-class-table td.name{font-size:11.5pt;line-height:1.22}.class-list-doc.class-list-students-few .rank-cell{font-size:14pt}.class-list-doc.class-list-students-medium .result-table{font-size:9.7pt}.class-list-doc.class-list-students-medium .result-table th,.class-list-doc.class-list-students-medium .result-table td{padding:1.65mm .65mm;height:8mm}.class-list-doc.class-list-students-many .result-table{font-size:8.2pt}.class-list-doc.class-list-students-many .result-table th,.class-list-doc.class-list-students-many .result-table td{padding:1mm .45mm;height:6.2mm}.official-student-bulletin-doc{padding:0 .5mm}.official-student-bulletin-doc .official-head{grid-template-columns:minmax(0,1fr) 42mm minmax(0,1fr);gap:6mm;font-size:10.8pt;line-height:1.35;margin-bottom:2.8mm}.official-student-bulletin-doc .official-side{gap:.55mm}.official-student-bulletin-doc .official-fr{font-size:10.8pt}.official-student-bulletin-doc .official-ar{font-size:11.7pt;line-height:1.36}.official-student-bulletin-doc .official-fr .official-strong{font-size:11.4pt}.official-student-bulletin-doc .official-ar .official-strong{font-size:12pt}.official-student-bulletin-doc .official-strong{font-weight:800}.official-student-bulletin-doc .official-center img{width:31mm;height:31mm}.official-student-bulletin-doc .basmala{font-size:11pt;margin-bottom:.8mm}.official-student-bulletin-doc .report-title{font-size:19pt;margin:1.3mm 0 2mm}.official-student-bulletin-doc .student-name{font-size:12.2pt;margin-top:.8mm;font-weight:700}.official-student-bulletin-doc .student-name>span{padding:1.8mm 2mm}.official-student-bulletin-doc .student-name b{font-size:11pt}.official-student-bulletin-doc .student-info{font-size:9.3pt;margin-bottom:2mm}.official-student-bulletin-doc .student-info .dual-info{padding:1.15mm 1.35mm;min-height:6.8mm}.official-student-bulletin-doc .student-info b{font-size:9.5pt}.official-student-bulletin-doc .secondary-bulletin-table{font-size:9.5pt;border-width:1.35px}.official-student-bulletin-doc .secondary-bulletin-table th{padding:1.2mm .42mm;line-height:1.12}.official-student-bulletin-doc .secondary-bulletin-table td{padding:1.12mm .45mm;height:8.4mm;font-size:9.4pt}.official-student-bulletin-doc .secondary-bulletin-table td:not(.name):not(.observation-cell){font-size:10.1pt;font-weight:700}.official-student-bulletin-doc .secondary-bulletin-table .discipline-head{width:28%}.official-student-bulletin-doc .secondary-bulletin-table .test-head,.official-student-bulletin-doc .secondary-bulletin-table .test-cell{width:9%}.official-student-bulletin-doc .secondary-bulletin-table .assessment-col,.official-student-bulletin-doc .secondary-bulletin-table .assessment-cell{width:9%}.official-student-bulletin-doc .secondary-bulletin-table .mean-head,.official-student-bulletin-doc .secondary-bulletin-table .mean-cell{width:11%}.official-student-bulletin-doc .secondary-bulletin-table .coef-head,.official-student-bulletin-doc .secondary-bulletin-table .coef-cell{width:6%}.official-student-bulletin-doc .secondary-bulletin-table .weighted-head,.official-student-bulletin-doc .secondary-bulletin-table .weighted-cell{width:11%}.official-student-bulletin-doc .secondary-bulletin-table .obs-head,.official-student-bulletin-doc .secondary-bulletin-table .observation-cell{width:25%}.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .discipline-head{width:24%}.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .test-head,.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .test-cell{width:8%}.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .assessment-col,.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .assessment-cell{width:8%}.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .mean-head,.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .mean-cell{width:9%}.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .coef-head,.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .coef-cell{width:6%}.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .weighted-head,.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .weighted-cell{width:9%}.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .obs-head,.official-student-bulletin-doc .secondary-bulletin-table[data-term="3"] .observation-cell{width:26%}.official-student-bulletin-doc .secondary-bulletin-table .name .dual-ar{font-size:9.8pt;font-weight:800;line-height:1.14}.official-student-bulletin-doc .secondary-bulletin-table .name .dual-fr{font-size:8.4pt;font-weight:700;line-height:1.1}.official-student-bulletin-doc .secondary-bulletin-table th .dual-ar{font-size:8.7pt;font-weight:800;line-height:1.14}.official-student-bulletin-doc .secondary-bulletin-table th .dual-fr{font-size:7.4pt;font-weight:700;line-height:1.1;margin-top:.35mm}.official-student-bulletin-doc .secondary-bulletin-table .observation-cell{white-space:normal;overflow-wrap:anywhere;word-break:normal;line-height:1.16;padding:1mm .7mm}.official-student-bulletin-doc .secondary-bulletin-table .observation-cell .dual-ar{display:block;font-size:9.3pt;font-weight:800;line-height:1.16}.official-student-bulletin-doc .secondary-bulletin-table .observation-cell .dual-fr{display:block;font-size:8pt;font-weight:700;line-height:1.12;margin-top:.35mm}.official-student-bulletin-doc .official-bulletin-summary{display:grid;grid-template-columns:1fr 1.2fr .72fr 1.28fr;border:1.35px solid #111;border-top:0}.official-student-bulletin-doc .official-bulletin-summary>div{min-height:14mm;padding:1.7mm 1.2mm;border-inline-end:1px solid #222;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1mm;text-align:center;overflow:hidden}.official-student-bulletin-doc .official-bulletin-summary>div:last-child{border-inline-end:0}.official-student-bulletin-doc .official-bulletin-summary b{display:block;font-size:9.2pt;line-height:1.16;font-weight:800}.official-student-bulletin-doc .official-bulletin-summary b .dual-ar,.official-student-bulletin-doc .official-bulletin-summary b .dual-fr{display:block;line-height:1.14}.official-student-bulletin-doc .official-bulletin-summary b .dual-fr{font-size:.84em;margin-top:.35mm}.official-student-bulletin-doc .official-bulletin-summary strong{display:block;font-size:12.2pt;line-height:1.18;font-weight:800}.official-student-bulletin-doc .official-bulletin-summary strong .dual-ar,.official-student-bulletin-doc .official-bulletin-summary strong .dual-fr{display:block;line-height:1.16;white-space:normal}.official-student-bulletin-doc .official-bulletin-summary strong .dual-fr{font-size:.82em;margin-top:.55mm}.official-student-bulletin-doc .director-observations{border:1.35px solid #111;border-top:0;min-height:23mm;padding:2mm 2.5mm;display:grid;grid-template-columns:1fr 1fr;gap:6mm}.official-student-bulletin-doc .director-observations .obs-label{font-size:9.2pt;font-weight:800;line-height:1.15}.official-student-bulletin-doc .director-observations .obs-lines{border-bottom:1px dotted #555;height:6mm;margin-top:1.5mm}.official-student-bulletin-doc .official-signatures{margin:7mm 8mm 0;gap:12mm}.official-student-bulletin-doc .official-signatures b{font-size:9.5pt}.official-student-bulletin-doc .official-signatures span{font-size:8pt}.official-student-bulletin-doc .official-signatures small{font-size:7.2pt}.official-student-bulletin-doc .official-signatures i{width:36mm;height:7mm}.student-page{min-height:281mm;padding:0 1mm;break-after:page;page-break-after:always;break-inside:avoid;page-break-inside:avoid}.student-page:last-child{break-after:auto;page-break-after:auto}.two-up{padding:0 1.5mm}.student-copy{height:138.2mm;overflow:hidden;padding:0 1mm;break-inside:avoid;page-break-inside:avoid}.student-copy .official-head{grid-template-columns:minmax(0,1fr) 27mm minmax(0,1fr);gap:3.5mm;font-size:7.1pt;line-height:1.22;margin-bottom:1mm}.student-copy .basmala{font-size:7.2pt;margin-bottom:.35mm}.student-copy .official-center img{width:17.5mm;height:17.5mm}.student-copy .report-title{font-size:10.8pt;margin:.45mm 0 .6mm;gap:2.2mm}.student-copy .student-name{font-size:6.9pt;margin:.25mm 0 0}.student-copy .student-name>span{padding:.45mm .8mm}.student-copy .student-info{font-size:5.5pt;margin:0 0 .45mm;gap:0}.student-copy .student-info .dual-info{padding:.3mm .55mm;min-height:3.5mm}.student-copy .secondary-bulletin-table{font-size:5.6pt}.student-copy .secondary-bulletin-table thead th{padding:.28mm .15mm;line-height:1}.student-copy .secondary-bulletin-table .group-head{font-size:5.8pt}.student-copy .secondary-bulletin-table td{padding:.32mm .16mm;height:4.1mm}.student-copy .secondary-bulletin-table .name .dual-fr,.student-copy .secondary-bulletin-table th .dual-fr{font-size:.78em;margin-top:.15mm}.student-copy .secondary-summary{margin-top:.65mm}.student-copy .secondary-summary>div{gap:1mm;padding:.45mm .7mm;font-size:5.9pt}.student-copy .secondary-summary strong{font-size:6.8pt}.student-copy .secondary-observation{margin-top:.65mm;padding:.4mm .7mm;gap:1.5mm;font-size:5.8pt}.student-copy .incomplete{font-size:5.1pt;margin:.4mm 0;gap:.2mm}.student-copy .official-signatures{grid-template-columns:1fr .65fr 1fr;gap:5mm;margin:.8mm 5mm 0}.student-copy .official-signatures b{font-size:5.8pt}.student-copy .official-signatures span{font-size:5pt}.student-copy .official-signatures small{font-size:4.6pt}.student-copy .official-signatures i{width:22mm;height:2.4mm}.student-copy .official-date-line{padding-top:.4mm}.student-copy .stamp-zone{width:16mm;height:4.8mm;font-size:4.4pt;margin:.15mm 0}.student-copy .director-zone i{height:1.6mm}.cut-line{height:3.4mm;display:flex;align-items:center;gap:2mm;color:#666;font-size:6.4pt}.cut-line::before,.cut-line::after{content:"";flex:1;border-top:1px dashed #777}.cut-line span{white-space:nowrap}.student-pair{break-after:page;page-break-after:always}.student-pair:last-child{break-after:auto;page-break-after:auto}.landscape-doc .official-head{grid-template-columns:minmax(0,1fr) 30mm minmax(0,1fr);font-size:8pt}.landscape-doc .official-center img{width:20mm;height:20mm}@media print{button{display:none}.paper{max-width:none}.result-table tr{break-inside:avoid}.official-head,.official-signatures,.student-name,.student-copy{break-inside:avoid}.two-up{page-break-after:avoid}}
...
function studentBulletin(data,row){
 if(professorStudentFullyAbsent(row)){toast(tr('هذا التلميذ غائب عن الاختبار والامتحان في جميع المواد؛ يظهر في اللائحة فقط ولا يُنشأ له كشف فردي.','Cet élève est absent à l’interrogation et à la composition dans toutes les matières ; il apparaît uniquement dans la liste collective.'));return}
...
function printAllStudentBulletins(data){
 const allRows=Array.isArray(data?.students)?data.students:[],rows=allRows.filter(row=>!professorStudentFullyAbsent(row)),skipped=allRows.length-rows.length;
 if(!allRows.length){toast(tr('لا يوجد تلاميذ للطباعة','Aucun élève à imprimer'));return}
 if(!rows.length){toast(tr('جميع التلاميذ غائبون عن الاختبار والامتحان في كل المواد؛ سيظهرون في اللائحة الجماعية فقط.','Tous les élèves sont absents à l’interrogation et à la composition dans toutes les matières ; ils apparaissent uniquement dans la liste collective.'));return}
 if(skipped>0)toast(tr('تم استبعاد '+skipped+' من الكشوف الفردية بسبب الغياب الكامل، مع بقائهم في اللائحة الجماعية.',''+skipped+' élève(s) absent(s) sont exclus des bulletins individuels et restent dans la liste collective.'));
...
function printClassList(data){
 ...
 const rows=orderedStudents.map(row=>{const studentName=profStudentNamePair(row.student),remark=professorRemarkPair(row.general),rank=Number(row.rank);return `<tr><td class="rank-cell">${Number.isFinite(rank)&&rank>=1?esc(String(rank)):'—'}</td><td class="name">${dualReportLabel(studentName.ar||row.student.name,studentName.fr||row.student.name)}${row.student.nns?`<small dir="ltr">NNS: ${esc(row.student.nns)}</small>`:''}</td>${row.subjectResults.map(r=>`<td class="subject-value">${professorCollectiveSubjectCell(r,data.term)}</td>`).join('')}<td class="general-value">${compactResultText(row.general)}</td><td class="name appreciation-cell">${dualReportLabel(remark.ar,remark.fr)}</td></tr>`}).join('');
 const avgAr=data.term===3?(data.curriculumComplete?'المعدل العام':'المعدل العام المؤقت'):(data.curriculumComplete?'معدل الفصل':'معدل الفصل المؤقت'),avgFr=data.term===3?(data.curriculumComplete?'Moyenne générale':'Moyenne générale provisoire'):(data.curriculumComplete?'Moyenne du trimestre':'Moyenne provisoire du trimestre');
 const body=`<div class="report-subtitle collective-meta"><div class="collective-meta-row ar" dir="rtl"><span><b>القسم:</b> ${esc(className.ar)}</span><span><b>الفصل:</b> ${esc(term.ar)}</span><span><b>مجموع المعاملات:</b> ${curriculumProgressText(data)}</span></div><div class="collective-meta-row fr" dir="ltr"><span><b>Classe :</b> ${esc(className.fr)}</span><span><b>Trimestre :</b> ${esc(term.fr)}</span><span><b>Total coefficients :</b> ${curriculumProgressText(data)}</span></div></div>${!data.curriculumComplete?`<p class="incomplete"><span dir="rtl">${esc(notice.ar)}</span><span dir="ltr">${esc(notice.fr)}</span></p>`:''}<table class="result-table collective-class-table"><thead><tr><th class="rank-head edge-vertical-head"><span class="edge-vertical-label"><b dir="rtl">${esc(data.curriculumComplete?'الرتبة':'رتبة مؤقتة')}</b><small dir="ltr">${esc(data.curriculumComplete?'Rang':'Rang provisoire')}</small></span></th><th class="student-head">${dualReportLabel('التلميذ','Élève')}</th>${subjectHeads}<th class="average-head">${dualReportLabel(avgAr,avgFr)}</th><th class="appreciation-head edge-vertical-head"><span class="edge-vertical-label"><b dir="rtl">التقييم</b><small dir="ltr">Appréciation</small></span></th></tr></thead><tbody>${rows}</tbody></table>`;
...
async function renderResults(classId='',term=1){
 ...
 const switcher=professorReportSwitcher('collective');
 if(!classes.length){
