...
function openGrades(id,term=1){
 ...
 const cards=students.map((s,i)=>{
  ...
  return `<article class="prof-grade-student-card" data-prof-grade-card="${esc(s.id)}">
   ...
   <footer class="prof-grade-student-results"><span><small>${esc(avgHead)}</small><b data-avg="${esc(s.id)}">${avg==null?'—':avg.toFixed(2)}${avg==null?'':'/20'}</b></span>${showAnnual?`<span><small>${tr('المعدل العام للمادة','Moyenne générale de la matière')}</small><b data-annual="${esc(s.id)}">${annualAvg==null?'—':annualAvg.toFixed(2)}${annualAvg==null?'':'/20'}</b></span>`:''}</footer>
  </article>`
 }).join('');
...
function professorSecondaryBulletinTable(data,row){
 ...
 return `<table class="result-table secondary-bulletin-table" data-term="${term}"><thead><tr><th class="discipline-head">${dualReportLabel('المواد الدراسية','Disciplines')}</th><th>${dualReportLabel('الاختبار /20','Interrogation /20')}</th><th class="assessment-col">${dualReportLabel(term===3?'الامتحان النهائي /20':'امتحان الفصل /20',term===3?'Examen final /20':'Composition /20')}</th><th class="mean-head">${dualReportLabel(avgLabel[0],avgLabel[1])}</th>${annual?`<th class="mean-head">${dualReportLabel('المعدل العام للمادة','Moyenne finale')}</th>`:''}<th class="coef-head">${dualReportLabel('المعامل','Coef.')}</th><th class="weighted-head">${dualReportLabel('النقاط الموزونة','Points pondérés')}</th><th class="obs-head">${dualReportLabel('ملاحظات الأستاذ','Observations du professeur')}</th></tr></thead><tbody>${rows||`<tr><td colspan="${colspan}">${dualReportLabel('لا توجد مواد','Aucune matière')}</td></tr>`}</tbody></table>`
}
