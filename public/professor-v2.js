...
function annualSubjectResult(m){
 const r1=termRecord(m,1),r2=termRecord(m,2),r3=termRecord(m,3);
...
function topbar(title='',subtitle='',home=false){
 ...
 return `<header class="professor-topbar professor-reference-topbar">
  <div class="professor-reference-main">
   <div class="professor-account"><span class="prof-account-avatar">♙</span><span><b>${esc(accountName)}</b><small>${tr('أستاذ','Professeur')}${profSchool()?' · '+esc(profSchool()):''}</small></span></div>
  ...
  ${home?`<div class="prof-home-context"><button type="button" id="profHomeYear" class="prof-home-context-item"><span>▣</span><div><small>${tr('السنة الدراسية','Année scolaire')}</small><b dir="ltr">${esc(profile.year||'…')}</b></div></button><label class="prof-home-context-item prof-home-class-select"><span>♙</span><div><small>${tr('القسم الحالي','Classe actuelle')}</small><select id="profHomeClass">${classOptions||`<option value="">${tr('لا يوجد قسم','Aucune classe')}</option>`}</select></div></label></div>`:''}
 </header>`
}
function bindTop(el){
...
function renderHome(){
 ...
 <section class="prof-v2-hero prof-reference-hero"><div class="prof-home-hero-art" aria-hidden="true"><b>▤</b><span>☆</span></div><div class="prof-hero-copy"><small class="prof-home-kicker">${tr('لوحة النتائج','Tableau des résultats')}</small><h1>${tr('مرحباً، ','Bonjour, ')}${esc(profName(professorUser?.name||''))}</h1><p>${tr('تابع إدخال النتائج واكتمالها من مكان واحد.','Suivez la saisie et l’avancement des résultats depuis un seul endroit.')}</p><button class="primary" id="profGoGrades"><span>✎</span>${tr('إدخال النتائج','Saisir les résultats')}</button></div></section>
 <section class="prof-v2-stats prof-reference-stats"><article class="students"><span class="prof-stat-icon">♙</span><div><strong>${m.students.length}</strong><small>${tr('التلاميذ','Élèves')}</small></div></article><article class="average"><span class="prof-stat-icon">▥</span><div><strong class="prof-grade-outof" dir="ltr"><span>${averageText}</span><span>/20</span></strong><small>${tr('معدل القسم','Moyenne de la classe')}</small><em>${tr('مؤقت حسب الدرجات المدخلة','Provisoire selon les notes saisies')}</em></div></article><article class="complete"><span class="prof-stat-icon">✓</span><div><strong>${m.completion}%</strong><small>${tr('اكتمال الدرجات','Notes complètes')}</small></div></article><article class="needs"><span class="prof-stat-icon">!</span><div><strong>${m.needs}</strong><small>${tr('أقل من 10/20','Sous 10/20')}</small><em>${tr('مؤقت حسب الدرجات المدخلة','Provisoire selon les notes saisies')}</em></div></article></section>
 <section class="professor-content prof-home-status"><div class="prof-home-status-head"><div><span class="prof-home-status-icon">▥</span><div><h2>${tr('حالة الفصل','État du trimestre')}</h2><p>${tr('نسبة إدخال الدرجات لكل مادة','Progression de la saisie pour chaque matière')} · ${tr('الفصل','T')} ${m.term}${className?' · '+esc(className):''}</p></div></div><button id="profShowAllGrades">${tr('عرض الكل','Tout afficher')}</button></div><div class="prof-home-subject-list">${statusRows||`<div class="professor-empty compact"><div>✎</div><h3>${tr('ابدأ بإضافة مادة','Ajoutez votre première matière')}</h3><p>${tr('بعد إضافة المادة ستظهر نسبة إدخال نتائجها هنا.','Après ajout, sa progression apparaîtra ici.')}</p><div class="prof-home-empty-actions"><button class="primary" id="profEmptyAdd">+ ${tr('إضافة مادة أو قسم','Ajouter une matière ou une classe')}</button><button id="profJoinClass">🔗 ${tr('الانضمام برمز القسم','Rejoindre avec un code')}</button></div></div>`}</div></section>
 ${nav('home')}</div>`;
...
function termFormula(term){
 ...
 const annual=tr('المعدل النهائي للمادة = (اختبار1 + اختبار2 + اختبار3 + امتحان1 + امتحان2×2 + الامتحان النهائي×3) ÷ 9.','Moyenne finale de la matière = (test1 + test2 + test3 + composition1 + composition2×2 + examen final×3) ÷ 9.');
...
function professorSubjectListTable(a,term){
 ...
 return`<table class="result-table professor-own-list-table" data-own-subject-list="${esc(a.id)}" data-term="${term}"><thead><tr><th>${dualReportLabel('رقم النداء','N° d’appel')}</th><th>${dualReportLabel('اسم التلميذ','Élève')}</th><th>${dualReportLabel('الاختبار /20','Interrogation /20')}</th><th>${dualReportLabel(term===3?'الامتحان النهائي /20':'امتحان الفصل /20',term===3?'Examen final /20':'Composition /20')}</th><th>${dualReportLabel(avgLabel[0],avgLabel[1])}</th>${annual?`<th>${dualReportLabel('المعدل النهائي للمادة','Moyenne finale de la matière')}</th>`:''}</tr></thead><tbody>${rows||`<tr><td colspan="${annual?6:5}">${dualReportLabel('لا يوجد تلاميذ','Aucun élève')}</td></tr>`}</tbody></table>`
}
...
function printProfessorMySubjectsList(classId,term){
 ...
 const body=`<div class="report-subtitle professor-own-list-meta"><div class="dual-line"><span dir="rtl"><b>الأستاذ(ة):</b> ${esc(teacher.ar||professorUser?.name||'—')} · <b>المواد:</b> ${esc(arNames||'—')}</span><span dir="ltr"><b>Professeur :</b> ${esc(teacher.fr||professorUser?.name||'—')} · <b>Matières :</b> ${esc(frNames||'—')}</span></div></div>${table}${term===3?`<p class="own-list-annual-note">${dualReportLabel('المعدل النهائي لكل مادة محسوب تلقائيًا من اختبارات الفصول الثلاثة والامتحانات بأوزان 1 و2 و3.','La moyenne finale de chaque matière est calculée automatiquement avec les examens pondérés 1, 2 et 3.')}</p>`:''}`;
...
function openMySubjectsList(classId,term=1){
...
function openSubjectList(id,term=1){
...
function openGrades(id,term=1){
 ...
 const cards=students.map((s,i)=>{
  ...
  return `<article class="prof-grade-student-card" data-prof-grade-card="${esc(s.id)}">
   ...
   <footer class="prof-grade-student-results"><span><small>${esc(avgHead)}</small><b data-avg="${esc(s.id)}">${avg==null?'—':avg.toFixed(2)}${avg==null?'':'/20'}</b></span>${showAnnual?`<span><small>${tr('المعدل النهائي للمادة','Moyenne finale de la matière')}</small><b data-annual="${esc(s.id)}">${annualAvg==null?'—':annualAvg.toFixed(2)}${annualAvg==null?'':'/20'}</b></span>`:''}</footer>
  </article>`
 }).join('');
...
function professorSecondaryBulletinTable(data,row){
 ...
 const rows=subjects.map((s,i)=>{
  const r=results[i]||{},name=profSubjectPair(s.subject,s.subjectKey,data.levelCode),remark=professorRemarkPair(annual?r.annualAverage:r.average);
 ...
 return `<table class="result-table secondary-bulletin-table" data-term="${term}"><thead><tr><th class="discipline-head">${dualReportLabel('المواد الدراسية','Disciplines')}</th><th>${dualReportLabel('الاختبار /20','Interrogation /20')}</th><th class="assessment-col">${dualReportLabel(term===3?'الامتحان النهائي /20':'امتحان الفصل /20',term===3?'Examen final /20':'Composition /20')}</th><th class="mean-head">${dualReportLabel(avgLabel[0],avgLabel[1])}</th>${annual?`<th class="mean-head">${dualReportLabel('المعدل النهائي للمادة','Moyenne finale')}</th>`:''}<th class="coef-head">${dualReportLabel('المعامل','Coef.')}</th><th class="weighted-head">${dualReportLabel('النقاط الموزونة','Points pondérés')}</th><th class="obs-head">${dualReportLabel('ملاحظات الأستاذ','Observations du professeur')}</th></tr></thead><tbody>${rows||`<tr><td colspan="${colspan}">${dualReportLabel('لا توجد مواد','Aucune matière')}</td></tr>`}</tbody></table>`
}
...
function professorSignatures(){
 return `<div class="official-signatures school-signatures">
  ...
  <div class="signature-zone director-zone"><b>المدير</b><span dir="ltr">Le Directeur</span><small>التوقيع والختم / Signature et cachet</small><div class="stamp-zone">الختم / Cachet</div><i></i></div>
 </div>`
}
...
function printAllStudentBulletins(data){
 ...
 printProfessorDocument(title,'',{className:data.className,term:data.term,subjectCount:data.subjects.length,titleAr:titlePair.ar,titleFr:titlePair.fr,layoutClass:'student-bulletin-doc',copyBodies:bodies})
}
...
function printClassList(data){
 ...
 printProfessorDocument(tr('اللائحة الجماعية للقسم','Liste collective de la classe'),body,{className:data.className,term:data.term,compact:true,subjectCount:data.subjects.length,titleAr:'اللائحة الجماعية للقسم',titleFr:'Liste collective de la classe',layoutClass:'class-list-doc',landscape:professorClassListLandscape(data.subjects?.length||0)})
}

function professorReportSwitcher(active='own'){return `<div class="prof-report-switcher"><button class="${active==='own'?'on':''}" data-report-section="own">▤ <b>${tr('لائحة موادي','Listes de mes matières')}</b><small>${tr('نتائج موادي فقط','Mes matières uniquement')}</small></button><button class="${active==='collective'?'on':''}" data-report-section="collective">👥 <b>${tr('النتائج الجماعية','Résultats collectifs')}</b><small>${tr('كشف جماعي + اللائحة','Bulletin collectif + liste')}</small></button></div>`}
...
function renderReportsHub(term=1){
 ...
 const classCards=displayClasses().map(cls=>({cls,subjects:assignmentsForClass(cls.id)})).filter(x=>x.subjects.length).map(({cls,subjects})=>{const names=subjects.map(a=>`<span>${esc(profSubject(a.subject))} <small>×${coefficientOf(a)}</small></span>`).join('');return `<article class="prof-report-subject-card prof-report-class-card"><div class="prof-report-subject-icon">▤</div><div><small>${esc(profClass(cls.name))}</small><h3>${tr('لائحة موادي','Liste de mes matières')}</h3><div class="prof-report-class-subjects">${names}</div></div><button data-open-own-class-report="${esc(cls.id)}">▤ ${tr('فتح اللائحة','Ouvrir la liste')}</button></article>`}).join('');
 el.innerHTML=`<div class="professor-shell">${topbar(tr('التقارير والطباعة','Rapports et impression'),tr('فضاء الأستاذ','Espace professeur'))}<section class="professor-content prof-reports-content">${professorReportSwitcher('own')}<div class="prof-term-tabs">${[1,2,3].map(t=>`<button class="${t===term?'on':''}" data-own-report-term="${t}">${tr('الفصل '+t,'Trimestre '+t)}</button>`).join('')}</div><div class="prof-report-section-title"><div><small>${tr('تقاريري الفردية','Mes rapports individuels')}</small><h2>${tr('لائحة موادي','Liste de mes matières')}</h2><p>${tr('كل موادك في القسم نفسه تظهر داخل لائحة واحدة وPDF واحد. لكل فصل اختبار واحد /20 وامتحان واحد /20، ويظهر المعدل النهائي للمادة في الفصل الثالث.','Toutes vos matières d’une même classe apparaissent dans une seule liste et un seul PDF. Chaque trimestre comporte un test /20 et une composition /20 ; la moyenne finale de la matière apparaît au troisième trimestre.')}</p></div></div><div class="prof-report-subject-grid">${classCards||`<div class="professor-empty compact"><h3>${tr('لا توجد مواد بعد','Aucune matière')}</h3><p>${tr('أضف مادة لتظهر لائحتها هنا.','Ajoutez une matière pour voir sa liste ici.')}</p></div>`}</div><aside class="prof-collective-entry"><div><b>👥 ${tr('النتائج الجماعية','Résultats collectifs')}</b><small>${shared?tr('تجمع جميع مواد الأساتذة المرتبطين، وتظهر المادة بدرجة 0 إلى أن يُدخل أستاذها الدرجات.','Regroupe toutes les matières des professeurs liés ; une matière reste à 0 tant que son professeur n’a saisi aucune note.'):tr('تظهر بعد ربط القسم بالأساتذة.','Disponibles après liaison de la classe.')}</small><div class="prof-collective-tags"><em>${tr('كشف جماعي','Bulletin collectif')}</em><em>${tr('اللائحة','Liste')}</em></div></div><button data-report-section="collective">${tr('فتح','Ouvrir')} ›</button></aside></section>${nav('reports')}</div>`;
 bindTop(el);bindNav(el);qa('[data-own-report-term]',el).forEach(b=>b.onclick=()=>renderReportsHub(Number(b.dataset.ownReportTerm)));qa('[data-open-own-class-report]',el).forEach(b=>b.onclick=()=>openMySubjectsList(b.dataset.openOwnClassReport,term));qa('[data-report-section="collective"]',el).forEach(b=>b.onclick=()=>renderResults('',term))
}
