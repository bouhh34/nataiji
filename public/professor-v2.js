...
function printProfessorDocument(title,body,opts={}){
 ...
 const className=opts.className||'',term=Number(opts.term)||1,compact=opts.compact?' compact':'',subjectCount=Math.max(0,Number(opts.subjectCount)||0),studentCount=Math.max(0,Number(opts.studentCount)||0),density=subjectCount&&subjectCount<=3?' few':subjectCount<=7?' medium':subjectCount?' dense':'',studentDensity=studentCount&&studentCount<=10?' class-list-students-few':studentCount<=20?' class-list-students-medium':studentCount?' class-list-students-many':'',twoCopies=opts.twoCopies===true,copyBodies=Array.isArray(opts.copyBodies)?opts.copyBodies.filter(Boolean):[],twoUp=twoCopies||copyBodies.length>0,landscape=opts.landscape===true,layoutClass=String(opts.layoutClass||'').trim();
...
function professorClassListLandscape(){return true}
