...
function renderReports(){
 const pupils=Array.isArray(state.pupils)?state.pupils:[],subjects=Array.isArray(state.subjects)?state.subjects:[],marks=Array.isArray(state.marks)?state.marks:[],setText=(selector,value)=>{const el=$(selector);if(el)el.textContent=value??''};
 if(!pupils.length){const sheet=$('#sheet');if(sheet)sheet.innerHTML='';const paper=$('#paperResults');if(paper)paper.innerHTML='';const list=$('#paperList');if(list)list.innerHTML='';return false}
 const selected=Math.max(0,Math.min(pupils.length-1,Number($('#student')?.value)||0)),p=pupils[selected]||[],c=calc(selected),rank=ranks()[selected],row=Array.isArray(marks[selected])?marks[selected]:subjects.map(()=>'');
 const sheet=$('#sheet');if(sheet)sheet.innerHTML=subjects.map((sub,j)=>`<tr><td>${esc(sub?.[0]||'')}</td><td>${row[j]??''}</td><td></td></tr>`).join('')+`<tr><td>المجموع</td><td>${c.sum}</td><td></td></tr><tr><td>المعدل</td><td>${c.avg.toFixed(1)}</td><td></td></tr><tr><td>الرتبة</td><td>${rank??'—'}</td><td></td></tr>`;
 setText('#sheetNns',p[0]||'');setText('#sheetName',p[1]||'');setText('#sheetSchool',state.school||'—');setText('#sheetRegion',state.region||'—');setText('#sheetInspection',state.inspection||'—');setText('#sheetYear',state.year||'');setText('#sheetTerm',state.term||'');
 const list=$('#paperList');if(list)list.innerHTML=pupils.map((x,i)=>`<tr><td>${i+1}</td><td>${esc(x?.[0]||'')}</td><td>${esc(x?.[1]||'')}</td><td>${esc(x?.[2]||'')}</td><td>${esc(x?.[3]||'')}</td></tr>`).join('');
 setText('#classReportTitle',`امتحان ${state.term||''} ${state.className||''}`);
 const paper=$('#paperResults'),rs=ranks();if(paper)paper.innerHTML=`<thead><tr><th>#</th><th>التلميذ</th>${subjects.map(sub=>`<th>${esc(sub?.[0]||'')}</th>`).join('')}<th>المعدل</th><th>الرتبة</th></tr></thead><tbody>${pupils.map((x,i)=>{const r=Array.isArray(marks[i])?marks[i]:subjects.map(()=>'');return `<tr><td>${i+1}</td><td>${esc(x?.[1]||'')}</td>${subjects.map((_,j)=>`<td>${r[j]??''}</td>`).join('')}<td>${calc(i).avg.toFixed(1)}</td><td>${rs[i]??'—'}</td></tr>`}).join('')}</tbody>`;
 try{window.nataijiFinalizeReports?.()}catch{}
 return true
}
