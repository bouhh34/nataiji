...
function zeroResultPupil(i){const c=calc(i);return Number(c?.sum)===0&&Number(c?.avg)===0}
function printOnly(type){
 if(type==='student'){
  const i=Number($('#student')?.value)||0;
  if(zeroResultPupil(i)){
   const isFr=localStorage.getItem('nataiji-lang')==='fr';
   modal(isFr?'Élève absent':'تلميذ غائب',`<p>${isFr?'Aucun relevé individuel n’est généré pour un élève dont le total et la moyenne sont nuls. Il reste présent dans la liste avec le statut « Absent ».':'لا يُنشأ كشف فردي للتلميذ الذي مجموعه ومعدله صفر. يبقى اسمه في اللائحة وتظهر حالته «غائب».'}</p>`);
   return false
  }
 }
 document.body.dataset.print=type;window.print();return true
}
