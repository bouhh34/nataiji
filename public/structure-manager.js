...
function structureModal(draft=null){
 ...
 const p=modal('الفصول الدراسية والأقسام',`<h3>الفصول الدراسية</h3><div id="termRows">${d.terms.length?d.terms.map((t,i)=>`<div class="subject-edit"><input data-term="${i}" value="${esc2(t)}"><button data-term-del="${i}">×</button></div>`).join(''):'<p class="message">لا يوجد فصل دراسي بعد.</p>'}</div><button id="addTerm">+ إضافة فصل دراسي</button><hr><h3>الأقسام</h3><div id="classRows">${d.classes.length?d.classes.map((c,i)=>`<div class="subject-edit"><input data-class-name="${i}" value="${esc2(c.name)}" placeholder="اسم القسم"><input data-class-code="${i}" value="${esc2(c.code||'')}" placeholder="رمز القسم"><button data-class-del="${i}">×</button></div>`).join(''):'<p class="message">لا يوجد قسم بعد.</p>'}</div><button id="addClass">+ إضافة قسم</button><button class="primary action">حفظ وتثبيت</button><p class="message"></p>`);
 const syncDraft=()=>{p.querySelectorAll('[data-term]').forEach(x=>d.terms[+x.dataset.term]=x.value.trim());p.querySelectorAll('[data-class-name]').forEach(x=>d.classes[+x.dataset.className].name=x.value.trim());p.querySelectorAll('[data-class-code]').forEach(x=>d.classes[+x.dataset.classCode].code=x.value.trim())};
