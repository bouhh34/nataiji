...
function subjectMax(x,j){for(const v of [x?.max,x?.maxMark,x?.maxScore,x?.[3],state?.subjectMax?.[j],state?.maxMarks?.[j]]){const n=Number(v);if(Number.isFinite(n)&&n>0)return n}return 20}
function termAverage(i,term){const row=termRows(term)?.[i],subs=state?.subjects||[];if(!Array.isArray(row)||!subs.length||!subs.every((_,j)=>row[j]!==''&&row[j]!=null))return null;if(subs.every((_,j)=>isAbsent(row[j])))return null;let sum=0,max=0;subs.forEach((x,j)=>{const v=row[j];if(!isAbsent(v)){const n=Number(v);if(Number.isFinite(n)){sum+=n;max+=subjectMax(x,j)}}});return max?sum*20/max:null}
...
function patchFinalStudent(){
  ...
  if(baseAvg){if(baseAvg.cells[0])baseAvg.cells[0].textContent='معدل الفصل الثالث';if(baseAvg.cells[2])baseAvg.cells[2].textContent='Moyenne du 3e trimestre';baseAvg.cells[1].innerHTML=`<strong dir="ltr">${fmt(a3)} / 20</strong>`}
...
function fillFinalTable(table){
  ...
  const maxOf=(s,j)=>subjectMax(s,j);
  ...
  table.innerHTML=`<thead><tr>
    ...
    ${subjects.map((s,j)=>`<th class="final-sub"><div class="final-vertical"><span>${esc(s?.[0]||'')}</span></div><small class="final-max" dir="ltr">/${maxOf(s,j)}</small></th>`).join('')}
