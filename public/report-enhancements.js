(()=>{
'use strict';
const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)],seal='/1280px-National_Seal_of_Mauritania.svg.png';
const D={basmala:'بسم الله الرحمن الرحيم',republic:'الجمهورية الإسلامية الموريتانية',ministry:'وزارة التربية وإصلاح النظام التعليمي',region:'',inspection:'',school:'',schoolNns:'',director:'',motto:'شرف - إخاء - عدل'};let C={...D};
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const lang=()=>localStorage.getItem('nataiji-lang')||'ar';const L=(a,f)=>lang()==='fr'?f:a;
async function getState(){try{let r=await fetch('/api/state',{credentials:'same-origin'});return r.ok?await r.json():{}}catch{return{}}}
async function putHeader(h){let d=await getState(),s=d.state||d||{};s.reportHeader=h;let r=await fetch('/api/state',{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({state:s})});if(!r.ok)throw 0;C=h;heads()}
function meta(){return{year:$('#yearTop')?.value||'',cls:$('#classTop')?.value||'',term:$('#term')?.value||''}}
function shortClass(v){return String(v||'').split(' - ')[0]||v||''}
function val(key,v){if(lang()!=='fr')return v;return C[key+'Fr']||window.state?.bilingual?.fields?.[key]||v}
function row(ar,fr,value,latin=false){if(!value)return'';return `<div class="doc-line"><span>${L(ar,fr)}</span><b${latin?' dir="ltr"':''}>${esc(value)}</b></div>`}
function head(){let m=meta();return `<div class="doc-head"><div class="doc-right"><div class="doc-primary">${esc(L(C.republic,'République Islamique de Mauritanie'))}</div><div>${esc(L(C.ministry,'Ministère de l’Éducation et de la Réforme du Système Éducatif'))}</div>${row('الإدارة الجهوية للتربية:','Direction régionale de l’Éducation :',val('region',C.region))}${row('المفتشية:','Inspection :',val('inspection',C.inspection))}${row('المدرسة:','École :',val('school',C.school))}${row('الرقم المدرسي:','N° scolaire :',C.schoolNns,true)}</div><div class="doc-center"><div class="doc-basmala">${esc(L(C.basmala,'Au nom d’Allah, le Tout Miséricordieux, le Très Miséricordieux'))}</div><img src="${seal}" alt="Mauritanie"></div><div class="doc-left"><div class="doc-motto">${esc(L(C.motto||D.motto,'Honneur - Fraternité - Justice'))}</div>${row('السنة الدراسية:','Année scolaire :',m.year,true)}${row('القسم:','Classe :',shortClass(m.cls),true)}${row('الفصل:','Trimestre :',m.term)}</div></div>`}
...
async function all(two=true){
 ...
 let opts=allOptions.filter(o=>{
   ...
   const sum=(state.subjects||[]).reduce((total,_,j)=>{
     const v=row[j];
     if(v===''||v==null||/^(غائب|غائبة|absent|absente|a)$/i.test(String(v).trim()))return total;
     const n=Number(v);return total+(Number.isFinite(n)?n:0)
   },0);
   const c=calc(idx);
