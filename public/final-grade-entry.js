...
const termAvg=(i,t)=>{const row=termRows(t)?.[i]||[];if(allAbsent(row))return null;const sum=state.subjects.reduce((a,_,j)=>a+numeric(row[j]),0);return sum*20/totalMax()};
...
const totalRow=rows.find(r=>/^(المجموع|Total)$/i.test(String(r.cells?.[0]?.textContent||'').trim()));const avgRow=rows.find(r=>/^(المعدل|Moyenne|Moyenne du 3e trimestre|معدل الفصل الثالث)$/i.test(String(r.cells?.[0]?.textContent||'').trim()));const rankRow=rows.find(r=>/^(الرتبة|Rang|الرتبة العامة|Rang général)$/i.test(String(r.cells?.[0]?.textContent||'').trim()));if(totalRow?.cells?.[1])totalRow.cells[1].innerHTML=`<strong dir="ltr">${Number(c.sum.toFixed(2))} / ${c.totalMax}</strong>`;if(avgRow?.cells?.[1])avgRow.cells[1].innerHTML=c.absentAll?`<strong>${absenceLabel(i)}</strong>`:`<strong dir="ltr">${c.avg.toFixed(1)} / 20</strong>`;if(rankRow?.cells?.[1])rankRow.cells[1].innerHTML=c.absentAll?'<strong>—</strong>':`<strong>${rank}</strong>`;const obsRow=rows.find(r=>/^(الملاحظة|Observation)$/i.test(String(r.cells?.[0]?.textContent||'').trim()));if(obsRow?.cells?.[1]&&c.absentAll)obsRow.cells[1].innerHTML=`<strong>${absenceLabel(i)} / ${absenceLabel(i,true)}</strong>`;root.classList.toggle('report-incomplete',false)}
