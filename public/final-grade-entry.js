...
const allAbsent=(row=[])=>!!state?.subjects?.length&&state.subjects.every((_,j)=>isAbsent(row?.[j]));
...
const termAvg=(i,t)=>{const row=termRows(t)?.[i]||[];if(!termComplete(i,t)||allAbsent(row))return null;let sum=0,max=0;state.subjects.forEach((_,j)=>{const v=row[j];if(!isAbsent(v)){sum+=numeric(v);max+=maxFor(j)}});return max?sum*20/max:null};
...
const currentCalc=i=>{const row=currentRows()?.[i]||[],complete=rowComplete(row),absentAll=allAbsent(row);let sum=0,max=0;state.subjects.forEach((_,j)=>{const v=row[j];if(filled(v)&&!isAbsent(v)){sum+=numeric(v);max+=maxFor(j)}});return{sum,avg:max?sum*20/max:null,totalMax:max||totalMax(),complete,absentAll}};

/* Core calculation: blank = incomplete, absent = entered and worth zero. */
calc=function(i){return currentCalc(i)};
ranks=function(){const vals=(state?.pupils||[]).map((_,i)=>currentCalc(i));return vals.map(v=>!v.complete||v.absentAll||v.avg==null?null:1+vals.filter(x=>x.complete&&!x.absentAll&&x.avg!=null&&x.avg>v.avg).length)};
...
function pupilFemale(i){const p=state?.pupils?.[i]||[],g=String(p?.[2]??p?.gender??'').trim().toLowerCase();return /أنثى|انثى|female|féminin|feminin|fille|f$/.test(g)}
function absenceLabel(i){return pupilFemale(i)?(isFr()?'Absente':'غائبة'):(isFr()?'Absent':'غائب')}
function patchStudentRoot(root,i){if(!root||!state?.pupils?.[i])return;const rows=qa('.sheet tbody tr',root),n=state.subjects.length,row=currentRows()?.[i]||[],c=currentCalc(i),rank=ranks()[i];for(let j=0;j<n&&j<rows.length;j++){const cell=rows[j]?.cells?.[1],v=row[j];if(!cell)continue;if(isAbsent(v))cell.innerHTML=`<strong class="absence-text">${absenceLabel(i)}</strong>`;else if(!filled(v))cell.innerHTML='<span>—</span>'}
const totalRow=rows.find(r=>/^(المجموع|Total)$/i.test(String(r.cells?.[0]?.textContent||'').trim()));const avgRow=rows.find(r=>/^(المعدل|Moyenne|Moyenne du 3e trimestre|معدل الفصل الثالث)$/i.test(String(r.cells?.[0]?.textContent||'').trim()));const rankRow=rows.find(r=>/^(الرتبة|Rang|الرتبة العامة|Rang général)$/i.test(String(r.cells?.[0]?.textContent||'').trim()));if(totalRow?.cells?.[1])totalRow.cells[1].innerHTML=c.absentAll?`<strong>${absenceLabel(i)}</strong>`:`<strong dir="ltr">${Number(c.sum.toFixed(2))} / ${c.totalMax}</strong>`;if(avgRow?.cells?.[1])avgRow.cells[1].innerHTML=c.absentAll?`<strong>${absenceLabel(i)}</strong>`:(c.complete&&c.avg!=null?`<strong dir="ltr">${c.avg.toFixed(1)} / 20</strong>`:`<strong>${incompleteLabel()}</strong>`);if(rankRow?.cells?.[1])rankRow.cells[1].innerHTML=c.complete&&!c.absentAll&&rank!=null?`<strong>${rank}</strong>`:'<strong>—</strong>';root.classList.toggle('report-incomplete',!c.complete)}
