...
function patchReportsNow(){const i=Math.max(0,q('#student')?.selectedIndex??0);patchStudentRoot(q('#officialSheet'),i);qa('#printBatch .batch-sheet').forEach((root,j)=>patchStudentRoot(root,j));patchClassTable(q('#paperResults'));patchFinalAnnualTable(q('#paperResults'));if(document.body.dataset.print==='portal'&&document.body.dataset.portalType==='class'){const t=q('#printPortal table');patchClassTable(t);patchFinalAnnualTable(t)}window.nataijiApplyEvaluation?.()}
