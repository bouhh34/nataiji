...
function patchClassTable(table){if(!table?.tBodies?.[0])return;const rows=[...table.tBodies[0].rows],rs=ranks();rows.forEach((r,i)=>{const raw=currentRows()?.[i]||[];state.subjects.forEach((_,j)=>{const c=r.cells?.[j+2],v=raw[j];if(!c)return;if(isAbsent(v))c.innerHTML=`<strong class="absence-text">${absenceLabel(i)}</strong>`});if(state.term!=='الفصل الثالث'){const avgIndex=2+state.subjects.length,rankIndex=avgIndex+1,cc=currentCalc(i);if(r.cells?.[avgIndex])r.cells[avgIndex].innerHTML=cc.complete?`<strong>${cc.avg.toFixed(1)}</strong>`:`<strong>${incompleteLabel()}</strong>`;if(r.cells?.[rankIndex])r.cells[rankIndex].innerHTML=cc.complete?`<strong>${rs[i]}</strong>`:'—'}}})}
