...
assert('PWA cache version current', /nataiji-shell-v76/.test(sw));
...
assert('collective list is ranked and always landscape', /function professorClassListLandscape\(\)\{return true\}/.test(professorV2) && /orderedStudents=\[\.\.\.\(data\.students\|\|\[\]\)\]\.sort/.test(professorV2) && /Number\(row\.rank\)/.test(professorV2) && /collective-class-table/.test(professorV2));
...
assert('professor v2 cache-busted asset is current', /professor-v2\.js\?v=50/.test(index));
