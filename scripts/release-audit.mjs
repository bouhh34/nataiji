...
assert('PWA cache version current', /nataiji-shell-v74/.test(sw));
...
assert('collective list is ranked and always landscape', /function professorClassListLandscape\(\)\{return true\}/.test(professorV2) && /orderedStudents=\[\.\.\.\(data\.students\|\|\[\]\)\]\.sort/.test(professorV2) && /professorRankPair\(row\.rank\)/.test(professorV2) && /collective-class-table/.test(professorV2));
assert('collective professor results include every official curriculum subject', /const officialSubjects=orderedSubjects\.map/.test(server) && /assigned:false,marks:\{\}/.test(server) && /subjects\.every\(s=>s\.assigned\)/.test(server));
...
assert('professor v2 cache-busted asset is current', /professor-v2\.js\?v=49/.test(index));
