...
async function professorAggregatedResults(userId,localClassId,term){
 ...
 const rows=students.map((student,index)=>{
  ...
  const subjectResults=subjects.map(subject=>{const markRow=subject.marks?.[student.id]||{},rec=professorTermRecord(markRow,term),average=professorTermAverage(markRow,term),weighted=average==null?null:average*subject.coefficient,testMean=rec.tests.some(v=>v==null)?null:rec.tests.reduce((a,b)=>a+b,0)/3,annualAverage=term===3?professorAnnualAverage(markRow):null;if(average==null)complete=false;else{completedSubjects++;weightedSum+=weighted;coefficientSum+=subject.coefficient}return{key:subject.key,average,weighted,tests:rec.tests,exam:rec.exam,testMean,annualAverage}});
