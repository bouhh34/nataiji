...
app.post('/api/subjects/normalize-primary',auth,async(req,res)=>{
 if(!pool)return res.status(503).json({error:'durable_storage_required'});
 if(req.user.role!=='admin')return res.status(403).json({error:'forbidden'});
 const st=await loadDurableSchoolStructure(req.user.schoolId),classId=String(req.body?.classId||st.activeClassId||''),cls=(st.classes||[]).find(x=>x.id===classId);
