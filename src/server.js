...
function preserveProfessorMarks(beforeInput,candidateInput){
 const before=cleanProfessorProfile(beforeInput),candidate=cleanProfessorProfile(candidateInput),beforeAssignments=new Map(before.assignments.map(a=>[String(a.id),a])),classStudents=new Map(candidate.classes.map(c=>[String(c.id),new Set((c.students||[]).map(st=>String(st.id)))]));
 candidate.marks=candidate.marks&&typeof candidate.marks==='object'?candidate.marks:{};
 for(const a of candidate.assignments){
  const id=String(a.id),old=beforeAssignments.get(id);if(!old||String(old.classId)!==String(a.classId))continue;
  const valid=classStudents.get(String(a.classId))||new Set(),oldRows=before.marks?.[id]&&typeof before.marks[id]==='object'?before.marks[id]:{},out=candidate.marks[id]&&typeof candidate.marks[id]==='object'?candidate.marks[id]:{};
  for(const [studentId,row] of Object.entries(oldRows))if(valid.has(String(studentId)))out[String(studentId)]=structuredClone(row);
  candidate.marks[id]=out
 }
 return cleanProfessorProfile(candidate)
}
async function appendProfessorGradeHistory(client,userId,assignmentId,studentId,term,record){
 await client.query('INSERT INTO nataiji_professor_grade_history(user_id,assignment_id,student_id,term,record,created_at) VALUES($1,$2,$3,$4,$5::jsonb,now())',[userId,assignmentId,studentId,term,JSON.stringify(record||{})])
}
...
async function archiveProfessorProfileVersion(userId,client,reason='save',data=null){
 ...
 await runner.query('DELETE FROM nataiji_professor_profile_versions WHERE user_id=$1 AND id NOT IN (SELECT id FROM nataiji_professor_profile_versions WHERE user_id=$1 ORDER BY id DESC LIMIT 200)',[userId]);
}
...
async function loadDurableSchoolStructure(schoolId){
 let legacy=null;
 try{const raw=await storeGet(schoolKey(schoolId));if(raw)legacy=JSON.parse(raw)}catch{}
 return repairStructureIfNeeded(schoolId,legacy)
}
